...
- **Profil** — upload foto profil, ubah nomor WA, ubah kata sandi sendiri.
- **Admin Dashboard** — ringkasan agregat, distribusi risiko per OPD, tabel hasil skrining (filter nama, cetak, hapus), manajemen permintaan JAGAD CARE (ubah status, hapus), kelola ASN (reset password, hapus akun), dan tombol kirim pengingat WhatsApp manual.
...
## 4. Foto Profil & Reset Password oleh Admin

### 4.1 Setup Storage bucket foto profil

1. Buka **SQL Editor** Supabase, tempel seluruh isi file `supabase/storage-avatars.sql`, lalu jalankan (RUN).
2. Ini membuat bucket `avatars` (public) beserta aturan agar tiap ASN hanya bisa mengubah fotonya sendiri.
3. Setelah ini, fitur "Ganti Foto" di halaman Profil otomatis berfungsi — tidak perlu langkah tambahan.

### 4.2 Deploy Edge Function reset password admin

Karena aplikasi ini tidak memakai email sungguhan, reset password lewat "link email" standar tidak dapat dipakai. Sebagai gantinya, admin bisa mereset password ASN langsung dari Admin Dashboard (tab **Kelola ASN**), lewat Edge Function berikut:

```bash
supabase functions deploy admin-reset-password
```

Tidak perlu secret tambahan — function ini otomatis memakai `SUPABASE_URL`, `SUPABASE_ANON_KEY`, dan `SUPABASE_SERVICE_ROLE_KEY` yang sudah tersedia otomatis di tiap Edge Function Supabase.

Setelah deploy, admin tinggal buka **Admin Dashboard → Kelola ASN**, klik **"Reset Password"** di baris ASN yang bersangkutan, masukkan kata sandi baru, lalu sampaikan ke ybs secara langsung/rahasia (misal lewat WhatsApp pribadi, bukan lewat sistem).

### 4.3 Izin hapus data (RLS) & Edge Function hapus akun

Fitur "Hapus" di Admin Dashboard (Data Skrining, Permintaan CARE, dan Kelola ASN) butuh dua hal tambahan:

1. **Izin hapus di database** — kalau database Anda sudah lebih dulu dibuat sebelum fitur ini ada, jalankan `supabase/rls-delete-policies.sql` di SQL Editor (kalau baru setup dari awal, sudah otomatis termasuk di `schema.sql`).
2. **Edge Function hapus akun ASN** — menghapus akun ASN sepenuhnya butuh hak admin API, tidak bisa langsung dari browser:
   ```bash
   supabase functions deploy admin-delete-asn
   ```

> **Peringatan:** menghapus akun ASN lewat tab Kelola ASN akan menghapus akun login beserta seluruh riwayat skrining & progres edukasinya secara permanen — tidak bisa dibatalkan. Admin harus mengetik "HAPUS" untuk konfirmasi sebelum tindakan ini dijalankan.
